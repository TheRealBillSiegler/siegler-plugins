...
- **The tiering skill leads with the ladder.** The design justification that opened the file folds into the scope line beside the standing rule. The skill speaks in its own voice, and verification dates move out of it entirely — the repo's coverage matrix owns them. The source post stays available to the installed session as context: the Source section points at the live post and the dated capture shipped in `references/`, with the live post outranking the capture and the doc anchors outranking both.
