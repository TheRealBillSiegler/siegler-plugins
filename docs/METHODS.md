...
- **Counts:** 6 HIGH, 7 MEDIUM, 4 LOW reported; 6 rejected-as-legitimate cases stated. All 6 HIGH confirmed on re-verification, 0 killed; two of them were drift introduced by earlier commits in the same PR (stale ledger paths in prose; the legacy `models` field dropped by the weekly summary) and four predated it. Of the 7 MEDIUM, **one was a false positive**: the audit reported that pages cited by the skills were absent from `check-drift.js`'s watch list, but the cited set and the watched set were already identical — the divergence it described did not exist. The underlying risk (nothing *keeps* them identical) was real and was acted on by deriving the watch list from the citations. All 13 HIGH and MEDIUM findings were acted on; the 4 LOW were not.
