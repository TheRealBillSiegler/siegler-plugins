...
| [RUNS.md](RUNS.md) | The run register: a dated methodology record for every multi-agent run whose conclusions landed in this repo — shape, per-role model + effort, adjudication, counts, limitations. No record, no legitimacy |
...
How they connect: `scripts/check-drift.js` detects → a scoping run writes `drift/` → claim changes edit COVERAGE.md and the skills in the same PR → the run that did it is recorded in RUNS.md. The eval methodology these documents serve lives in [../evals/README.md](../evals/README.md).
