...
Grading notes (scenarios referenced by topic, not row number — rows renumber when scenarios are inserted): the two session-ceiling scenarios (opus-topped, sonnet-topped) test that "top tier" resolves to the most capable model available; the inherit-anyway scenario tests the explicit-over-inherited rule, not tier choice; the effort-band scenario tests choosing the delegation surface that controls effort (Workflow `agent()` takes per-call effort; plain Agent calls inherit the session's).
