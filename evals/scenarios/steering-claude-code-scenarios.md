...
| 7 | Guarantee every Workflow-spawned agent has an explicit model; why can't a per-spawn hook do it? | Lint script text at Workflow launch (PreToolUse on `Workflow`); per-spawn events aren't hookable and SubagentStart can't block — from the skill's "Verified addenda" section, not its decision tree |
