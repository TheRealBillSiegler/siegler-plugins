...
Grading notes: #9 and #11 test that "top tier" resolves to the most capable model available, at two different session ceilings (opus-topped, sonnet-topped); #10 tests the explicit-over-inherited rule, not tier choice; #12 tests choosing the delegation surface that controls effort (Workflow `agent()` takes per-call effort; plain Agent calls inherit the session's).
