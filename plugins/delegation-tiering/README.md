...
Every claim in this plugin carries one of three provenance tiers — article digest, doc page, or dated live test — defined in the repo's [Anchoring policy](https://github.com/TheRealBillSiegler/claude-plugins/blob/main/docs/REMEDIATION.md#anchoring-policy). The skills' Doc anchors sections name the page behind each mechanic; where the docs are silent, the claim says so. Everything used to *build and measure* the plugin — its evals live at `evals/delegation-tiering/` — is in the [plugin repo](https://github.com/TheRealBillSiegler/claude-plugins) ([docs map](https://github.com/TheRealBillSiegler/claude-plugins/blob/main/docs/README.md)), not in the installed payload.
